...
class AuthService {
  ...
  final GoogleSignIn _googleSignIn = GoogleSignIn(
    ...
    serverClientId:
        '61632775478-536h573nva64bgbbegcfbhkpn169cnim.apps.googleusercontent.com',
  );
  ...
  Future<UserProfile?> register({
    ...
    try {
      ...
      await _supabase.rpc(
        ...
        params: {'user_id': response.user!.id, 'user_role': role},
      );
  ...
  Future<User?> signInWithGoogle() async {
    try {
      ...
      if (existingProfile == null) {
        ...
        await _supabase.rpc(
          'create_profile_for_user',
          params: {'user_id': response.user!.id, 'user_role': 'pending'},
        );
      }
  ...
  Future<ParentLinkCode> generateLinkingCode() async {
    if (currentUser == null) {
      throw Exception('User not logged in');
    }

    try {
      final response = await _supabase.rpc('generate_parent_link_code');

      if (response is! Map<String, dynamic>) {
        throw Exception('Invalid linking code response');
      }

      return ParentLinkCode.fromJson(response);
    } catch (e) {
      throw Exception('Failed to generate linking code: ${e.toString()}');
    }
  }
